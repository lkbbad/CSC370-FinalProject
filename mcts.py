...
def randomPolicy(state, visited):
    if state in visited:
        return -100000
    if state.isTerminal():
        return 10000

    return state.size * state.size - np.sum(state.state)
...
class mcts():
    ...
    def executeRound(self):
        ...
        reward = self.rollout(node.state, self.visited)
    ...
    def getBestChild(self, node, explorationValue, visited=None):
        bestValue = float("-inf")
        bestNodes = []
        for child in node.children.values():
            if visited is None or child.state not in visited:
